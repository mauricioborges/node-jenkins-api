...
JOBS_FROM_NESTED_VIEWS = API + '?tree=jobs[name]';
...
var init = exports.init = function(host) {
	...
	var view_url = function(views_list) {
    ...
    return {
        ...
        jobs_from_view: function(views,callback) {
			console.log("TESTE2:"+build_view_url(views,JOBS_FROM_NESTED_VIEWS))
            /*
            Return a list of object literals containing the name and color of all jobs on the Jenkins server
            */
            request({method: 'GET', url: build_url(view_url(views)+JOBS_FROM_NESTED_VIEWS)}, function(error, response, body) {
                if ( error || response.statusCode !== 200 ) {
                    callback(error || true, response, views);
                    return;
                }
                data = JSON.parse(body.toString()).jobs;
