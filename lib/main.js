...
JOBS_FROM_NESTED_VIEWS = API + '?tree=jobs[name,displayName]';
...
var init = exports.init = function(host) {
    ...
    return {
        ...
        jobs_from_view_old: function(views,callback) {
		...
		jobs_from_view: function(list_of_views, callback) {
		
				BUILDS_FROM_JOBS
				buildurl=build_url(view_url(list_of_views)+JOBS_FROM_NESTED_VIEWS)
				/*
				Return a list of object literals containing the name and color of all jobs on the Jenkins server
				*/
				request({method: 'GET', url: buildurl}, function(error, response, body) {
